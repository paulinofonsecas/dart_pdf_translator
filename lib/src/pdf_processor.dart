  ...
  Future<List<String>> extractText(
    ...
    try {
      ...
      if (firstPage < 1 || lastPage > pageCount || firstPage > lastPage) {
        throw ArgumentError(
            'Invalid page range: $firstPage-$lastPage for PDF with $pageCount pages.');
      }

      for (var i = firstPage; i <= lastPage; i += pagesPerBatch) {
        final int batchStart = i;
        final int batchEnd =
            (i + pagesPerBatch - 1).clamp(batchStart, lastPage);
        ...
        if (result.exitCode != 0) {
          print(
              'Warning: pdftotext failed for pages $batchStart-$batchEnd: ${result.stderr}');
      ...
      throw Exception(
          'An unexpected error occurred while processing the PDF with Poppler: $e');
    }
  }
}
